...
#ifndef SRC_MMWAVE_MODEL_MMWAVE_EXPRULE_MAC_SCHEDULER_H_
#define SRC_MMWAVE_MODEL_MMWAVE_EXPRULE_MAC_SCHEDULER_H_
...
class MmWaveFlexTtiExpruleMacScheduler : public MmWaveMacScheduler
{
public:
  ...
  MmWaveFlexTtiExpruleMacScheduler ();

  virtual ~MmWaveFlexTtiExpruleMacScheduler ();
  ...
  friend class MmWaveFlexTtiExpruleMacSchedSapProvider;
  friend class MmWaveFlexTtiExpruleMacCschedSapProvider;
...
private:
  ...
  static bool CompareUeWeightsExprule (FlowStats* lflow, FlowStats* rflow)
  {
    ...
    double lExpruleMetric = std::max (lflow->m_ueSchedInfo->m_currTputDl,lflow->m_ueSchedInfo->m_currTputUl) / std::max (1E-9,(lflow->m_ueSchedInfo->m_avgTputDl + lflow->m_ueSchedInfo->m_avgTputDl));
    double rExpruleMetric = std::max (rflow->m_ueSchedInfo->m_currTputDl,rflow->m_ueSchedInfo->m_currTputUl) / std::max (1E-9,(rflow->m_ueSchedInfo->m_avgTputDl + rflow->m_ueSchedInfo->m_avgTputDl));
    
    double lNewMetric = lExpruleMetric * exp((lAlpha * lRelDeadline) / (1 + sqrt(m_Y)));
    double rNewMetric = rExpruleMetric * exp((rAlpha * rRelDeadline) / (1 + sqrt(m_Y)));
...
#endif /* SRC_MMWAVE_MODEL_MMWAVE_EXPRULE_MAC_SCHEDULER_H_ */

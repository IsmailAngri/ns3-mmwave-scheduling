/*
 * A new scheduling algorithm for 5G NR networks
 *
 *  Created on: July 10, 2020
 *      Author: ANGRI Ismail
 */

#ifndef SRC_MMWAVE_MODEL_MMWAVE_MAXIMUMWEIGHT_MAC_SCHEDULER_H_
#define SRC_MMWAVE_MODEL_MMWAVE_MAXIMUMWEIGHT_MAC_SCHEDULER_H_
...
class MmWaveFlexTtiMaximumWeightMacScheduler : public MmWaveMacScheduler
{
public:
  ...
  MmWaveFlexTtiMaximumWeightMacScheduler ();

  virtual ~MmWaveFlexTtiMaximumWeightMacScheduler ();
  ...
  friend class MmWaveFlexTtiMaximumWeightMacSchedSapProvider;
  friend class MmWaveFlexTtiMaximumWeightMacCschedSapProvider;
...
private:
  ...
  static bool CompareUeWeightsMaximumWeight (FlowStats* lflow, FlowStats* rflow)
  {

    int lRelDeadline = lflow->m_deadlineUs - lflow->m_txQueueHolDelay;
    int rRelDeadline = rflow->m_deadlineUs - rflow->m_txQueueHolDelay;

    double lMaximumWeightMetric = std::max (lflow->m_ueSchedInfo->m_currTputDl,lflow->m_ueSchedInfo->m_currTputUl) / std::max (1E-9,(lflow->m_ueSchedInfo->m_avgTputDl + lflow->m_ueSchedInfo->m_avgTputDl));
    double rMaximumWeightMetric = std::max (rflow->m_ueSchedInfo->m_currTputDl,rflow->m_ueSchedInfo->m_currTputUl) / std::max (1E-9,(rflow->m_ueSchedInfo->m_avgTputDl + rflow->m_ueSchedInfo->m_avgTputDl));
    
    double lNewMetric = lRelDeadline * lMaximumWeightMetric;
    double rNewMetric = rRelDeadline * rMaximumWeightMetric;
...
#endif /* SRC_MMWAVE_MODEL_MMWAVE_MAXIMUMWEIGHT_MAC_SCHEDULER_H_ */

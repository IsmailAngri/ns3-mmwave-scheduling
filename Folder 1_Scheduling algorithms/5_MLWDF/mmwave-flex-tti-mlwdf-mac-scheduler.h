...
#ifndef SRC_MMWAVE_MODEL_MMWAVE_MLWDF_MAC_SCHEDULER_H_
#define SRC_MMWAVE_MODEL_MMWAVE_MLWDF_MAC_SCHEDULER_H_
...
class MmWaveFlexTtiMlwdfMacScheduler : public MmWaveMacScheduler
{
public:
  ...
  MmWaveFlexTtiMlwdfMacScheduler ();

  virtual ~MmWaveFlexTtiMlwdfMacScheduler ();
  ...
  friend class MmWaveFlexTtiMlwdfMacSchedSapProvider;
  friend class MmWaveFlexTtiMlwdfMacCschedSapProvider;
...
private:
  ...
  static bool CompareUeWeightsMlwdf (FlowStats* lflow, FlowStats* rflow)
  {

    std::list<double>::iterator lLargestDelayIt = std::max_element(lflow->m_txPacketDelays.begin (), lflow->m_txPacketDelays.end ());
    //std::list<double>::iterator lLargestDelayIt = lflow->m_txPacketDelays.begin (); 
    double lAlpha = (-log10((lflow->m_probErr)*pow((1-0.1),*lLargestDelayIt))) / *lLargestDelayIt;
    //double lAlpha = (-log10(0.01*pow((1-0.1),*lLargestDelayIt))) / *lLargestDelayIt;

    std::list<double>::iterator rLargestDelayIt = std::max_element(rflow->m_txPacketDelays.begin (), rflow->m_txPacketDelays.end ());
    //std::list<double>::iterator rLargestDelayIt = rflow->m_txPacketDelays.begin ();
    double rAlpha = (-log10((rflow->m_probErr)*pow((1-0.1),*rLargestDelayIt))) / *rLargestDelayIt;
    //double rAlpha = (-log10(0.01*pow((1-0.1),*rLargestDelayIt))) / *rLargestDelayIt;

    int lRelDeadline = lflow->m_deadlineUs - lflow->m_txQueueHolDelay;
    int rRelDeadline = rflow->m_deadlineUs - rflow->m_txQueueHolDelay;

    double lMlwdfMetric = std::max (lflow->m_ueSchedInfo->m_currTputDl,lflow->m_ueSchedInfo->m_currTputUl) / std::max (1E-9,(lflow->m_ueSchedInfo->m_avgTputDl + lflow->m_ueSchedInfo->m_avgTputDl));
    double rMlwdfMetric = std::max (rflow->m_ueSchedInfo->m_currTputDl,rflow->m_ueSchedInfo->m_currTputUl) / std::max (1E-9,(rflow->m_ueSchedInfo->m_avgTputDl + rflow->m_ueSchedInfo->m_avgTputDl));
    
    double lNewMetric = lAlpha * lRelDeadline * lMlwdfMetric;
    double rNewMetric = rAlpha * rRelDeadline * rMlwdfMetric;

    return(lNewMetric < rNewMetric);
  }
...
#endif /* SRC_MMWAVE_MODEL_MMWAVE_MLWDF_MAC_SCHEDULER_H_ */

...
#ifndef SRC_MMWAVE_MODEL_MMWAVE_EXPPF_MAC_SCHEDULER_H_
#define SRC_MMWAVE_MODEL_MMWAVE_EXPPF_MAC_SCHEDULER_H_
...
class MmWaveFlexTtiExppfMacScheduler : public MmWaveMacScheduler
{
public:
  ...
  MmWaveFlexTtiExppfMacScheduler ();

  virtual ~MmWaveFlexTtiExppfMacScheduler ();
  ...
  friend class MmWaveFlexTtiExppfMacSchedSapProvider;
  friend class MmWaveFlexTtiExppfMacCschedSapProvider;
...
private:
  ...
  static bool CompareUeWeightsExppf (FlowStats* lflow, FlowStats* rflow)
  {
    ...
    double lExppfMetric = std::max (lflow->m_ueSchedInfo->m_currTputDl,lflow->m_ueSchedInfo->m_currTputUl) / std::max (1E-9,(lflow->m_ueSchedInfo->m_avgTputDl + lflow->m_ueSchedInfo->m_avgTputDl));
    double rExppfMetric = std::max (rflow->m_ueSchedInfo->m_currTputDl,rflow->m_ueSchedInfo->m_currTputUl) / std::max (1E-9,(rflow->m_ueSchedInfo->m_avgTputDl + rflow->m_ueSchedInfo->m_avgTputDl));
    
    double lNewMetric = lExppfMetric * exp(((lAlpha * lRelDeadline) - m_X) / (1 + sqrt(m_X)));
    double rNewMetric = rExppfMetric * exp(((rAlpha * rRelDeadline) - m_X)/ (1 + sqrt(m_X)));
...
#endif /* SRC_MMWAVE_MODEL_MMWAVE_EXPPF_MAC_SCHEDULER_H_ */

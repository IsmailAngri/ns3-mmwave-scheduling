...
#ifndef SRC_MMWAVE_MODEL_MMWAVE_EXPMLWDF_MAC_SCHEDULER_H_
#define SRC_MMWAVE_MODEL_MMWAVE_EXPMLWDF_MAC_SCHEDULER_H_
...
class MmWaveFlexTtiExpmlwdfMacScheduler : public MmWaveMacScheduler
{
public:
  ...
  MmWaveFlexTtiExpmlwdfMacScheduler ();

  virtual ~MmWaveFlexTtiExpmlwdfMacScheduler ();
  ...
  friend class MmWaveFlexTtiExpmlwdfMacSchedSapProvider;
  friend class MmWaveFlexTtiExpmlwdfMacCschedSapProvider;
...
private:
  ...
  static bool CompareUeWeightsExpmlwdf (FlowStats* lflow, FlowStats* rflow)
  {

    //Calcul de Tho = LargestDelayIt
    ...
    //Calcul du rapport: débit_actuelle/débit_moyen
    double lExpmlwdfMetric = std::max (lflow->m_ueSchedInfo->m_currTputDl,lflow->m_ueSchedInfo->m_currTputUl) / std::max (1E-9,(lflow->m_ueSchedInfo->m_avgTputDl + lflow->m_ueSchedInfo->m_avgTputDl));
    double rExpmlwdfMetric = std::max (rflow->m_ueSchedInfo->m_currTputDl,rflow->m_ueSchedInfo->m_currTputUl) / std::max (1E-9,(rflow->m_ueSchedInfo->m_avgTputDl + rflow->m_ueSchedInfo->m_avgTputDl));
    
    double lNewMetric = (lAlpha * lExpmlwdfMetric) * (exp(*lLargestDelayIt / (*lLargestDelayIt - lRelDeadline))) ;
    double rNewMetric = (rAlpha * rExpmlwdfMetric) * (exp(*rLargestDelayIt / (*rLargestDelayIt - rRelDeadline))) ;
...
#endif /* SRC_MMWAVE_MODEL_MMWAVE_EXPMLWDF_MAC_SCHEDULER_H_ */

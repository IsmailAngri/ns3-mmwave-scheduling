...
*   -----------------------------------------
*   Modified on Jul 10, 2020 by: ANGRI Ismail
*   <ismail.angri@gmail.com>
*   To support new model for 5G NR scheduling
*/
...
//	LogComponentEnable ("LteRlcAm", LOG_LEVEL_LOGIC);
  ...
  std::string channelState = "l";
  bool smallScale = true;
  //double speed = 20.0;   // m/s
  double enbTxPowerDbm = 30.0;    // in dBm
  ...
  //------------
  //Ismail: ajouter speed et TxPower à la commande cmd
  ...
  Config::SetDefault ("ns3::MmWaveFlexTtiPfMacScheduler::SymPerSlot", UintegerValue (6));
  ...
  for (uint16_t i = 0; i < numEnb; i++)
    {
      ...
      //Ismail cout
      std::cout << "************* Position affected to gNB N° " << i << " *************" << std::endl;
    }
  MobilityHelper enbMobility;
  enbMobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  enbMobility.SetPositionAllocator (enbPositionAlloc);
  ...
  for (unsigned i = 0; i < numUe; i++)
    {
      ...
      std::cout << "*Position affected to UE N°" << i << " **is**" << dist << "m" << std::endl;
    }
...
//		}
  ...
  //enum EpsBearer::Qci q = EpsBearer::NGBR_LOW_LAT_EMBB_AR;
  //enum EpsBearer::Qci q = EpsBearer::NGBR_IMS;
  //EpsBearer bearer (q);
